...
public class PlanetSkyRenderer extends DimensionSpecialEffects {
    private final float[] sunriseCol = new float[4];
    private final float[] rainSizeX = new float[1024];
    private final float[] rainSizeZ = new float[1024];
    private Planet planet;
    private VertexBuffer starBuffer;
    public PlanetSkyRenderer(float cloudLevel, Planet planet) {
    ...
    public float[] getSunriseColor(float time, float partialTick) {
        ...
        if (planet.sky().sunriseColor().get(0) < 0) {
            sunriseCol[0] = 0;
            sunriseCol[1] = 0;
            sunriseCol[2] = 0;
            sunriseCol[3] = 0;
        ...
        float dr = planet.sky().sunriseColor().get(0);
        float dg = planet.sky().sunriseColor().get(1);
        float db = planet.sky().sunriseColor().get(2);
